...
if __name__ == "__main__":
	...
	while True:
		...
		if choice == "Y":
			...
			detail = str ( "Shared Client : |" ) + str ( iteration ) + " : |" + str ( argument ) + " . \n"
			log.write ( detail )

		elif choice == "N" or choice == "E":
			synchronize ( clientUDPSocket, addr )
			detail = str ( "Shared Client : |" ) + str ( iteration ) + " : |" + str ( "synchronize" ) + " . \n"
			log.write ( detail )
			continue

		else:
			message = "Error: Not a Proper input ! "
			detail = str ( "Shared Client : |" ) + str ( iteration ) + " : |" + str ( message ) + " . \n"
			log.write ( detail )
			print message
			continue

		if argument == "close":
			print "***************** Shared Directory Sync Closed *****************"
			clientSocket.close ( )
